...
function pickWorldPosition(screenPos) {
  // NOTE: this used to also reject picks whose primitive wasn't exactly
  // chimTileset, to stop defects landing on the globe in Satellite/OSM
  // view. That check relied on an assumption about what property holds
  // the tileset reference on a picked Cesium3DTileFeature, which could
  // not be verified against a live Cesium session from this sandbox —
  // and it turned out to reject valid, direct clicks on the model too,
  // not just the globe. Reverted to the simple version below, which was
  // the known-working behavior for the core drawing flow before that
  // change. The Satellite/OSM globe issue is real but secondary to
  // drawing working at all — needs a different fix, ideally verified
  // interactively rather than guessed at from here.
...
function captureDefectCanvasBlob(defect, callback) {
  ...
  setTimeout(() => {
    ...
    // Re-centering correction: getStraightOnView's camera math (surface
    // normal + ray casting) generally lands the target close to center,
    // but not always pixel-exact — the shape centroid computation it
    // feeds from has its own documented small tolerance (see
    // computeShapeCentroid), and that residual shows up here as the
    // defect sitting slightly off-center in the raw render. Rather than
    // chase that further in the 3D camera math, correct it directly:
    // ask Cesium exactly where the target point landed on screen (this
    // is the same projection Cesium itself just used to render it, so
    // it's authoritative, not another approximation) and crop the
    // output so that exact point sits at the image's center. Symmetric
    // crop around the target, sized to the largest region that still
    // fits inside the canvas — so if the target is already near-center
    // (the normal case), this trims only a small, usually invisible
    // margin off whichever side has less room.
    let srcX = 0, srcY = 0, srcW = W, srcH = H;
    try {
      const winPos = Cesium.SceneTransforms.worldToWindowCoordinates(chimViewer.scene, target);
      if (winPos && isFinite(winPos.x) && isFinite(winPos.y)) {
        const maxHalfW = Math.min(winPos.x, W - winPos.x);
        const maxHalfH = Math.min(winPos.y, H - winPos.y);
        // Only apply the crop if it still leaves a reasonably large
        // image — if the target is very near an edge (camera math went
        // badly wrong, or a degenerate shape), a tiny sliver crop would
        // make the capture worse, not better, so fall back to the full
        // frame instead in that case.
        if (maxHalfW > W * 0.3 && maxHalfH > H * 0.3) {
          srcX = Math.round(winPos.x - maxHalfW);
          srcY = Math.round(winPos.y - maxHalfH);
          srcW = Math.round(maxHalfW * 2);
          srcH = Math.round(maxHalfH * 2);
        }
      }
    } catch (_) { /* fall back to the uncropped full frame below */ }
    ...
    try {
      ctx.drawImage(cesiumCanvas, srcX, srcY, srcW, srcH, 0, 0, srcW, srcH);
    } catch(e) {
      ...
      chimViewer.zoomTo(chimTileset).then(() => {
        setTimeout(() => {
          try {
            ...
            chimViewer.scene.render();
            chimViewer.scene.render();
            // zoomTo's default view fits the whole chimney into frame, but
            // the canvas itself is wide (matches the browser viewport) —
            // so a tall, narrow structure like a chimney ends up as a
            // vertical strip surrounded by empty space on both sides.
            // Crop that down to a portrait-oriented image centered on the
            // chimney, rather than shipping the whole wide, mostly-empty
            // frame to the report. Full canvas height is kept (nothing
            // above/below the chimney gets cut off), only the sides are
            // trimmed in.
            const srcCanvas = chimViewer.scene.canvas;
            const targetAspect = 3 / 4; // width:height — portrait
            const cropW = Math.min(srcCanvas.width, Math.round(srcCanvas.height * targetAspect));
            const cropX = Math.round((srcCanvas.width - cropW) / 2);

            const outCanvas = document.createElement('canvas');
            outCanvas.width = cropW;
            outCanvas.height = srcCanvas.height;
            const ctx = outCanvas.getContext('2d');
            ctx.drawImage(srcCanvas, cropX, 0, cropW, srcCanvas.height, 0, 0, cropW, srcCanvas.height);
            outCanvas.toBlob((blob) => resolve(blob || null), 'image/jpeg', 0.92);
...
/* ── Download Markings as KML ─────────────────────────────────────────────── */

// Builds a KML file from the defects currently marked on this chimney and
// triggers a browser download — entirely client-side, since chimDefects
// already has everything needed (position, title, severity, etc.) once the
// project page has loaded, no server round-trip required.
function _kmlEscape(str) {
  return String(str == null ? '' : str)
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

function downloadMarkingsKml() {
  if (!chimDefects.length) {
    showHint('No markings to download yet — add a finding first.', 2400);
    return;
  }

  const placemarks = chimDefects.map(d => {
    const lon = d.position ? d.position.lon : null;
    const lat = d.position ? d.position.lat : null;
    const height = d.position ? d.position.height : 0;
    if (lon == null || lat == null) return '';

    const name = _kmlEscape(d.title || 'Finding');
    const descParts = [
      d.defect_type ? `Type: ${_kmlEscape(d.defect_type)}` : '',
      d.severity ? `Severity: ${_kmlEscape(d.severity)}` : '',
      d.status ? `Status: ${_kmlEscape(d.status)}` : '',
      d.location ? `Location: ${_kmlEscape(d.location)}` : '',
      d.notes ? `Notes: ${_kmlEscape(d.notes)}` : '',
    ].filter(Boolean);
  ...
  const projectName = _kmlEscape((window.CHIM_PROJECT_NAME || 'Chimney') + ' — Markings');
...
${placemarks}
  ...
  a.href = url;
  a.download = `${(window.CHIM_PROJECT_NAME || 'chimney').replace(/[^a-z0-9]+/gi, '_')}_markings.kml`;
