...
class ChimneyProject(db.Model):
    ...
    created_by      = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    created_at      = db.Column(db.DateTime, default=datetime.utcnow)
    # Client's own logo, uploaded when the project is created — shown
    # on the left side of every page of the inspection PDF report,
    # mirroring the Drogo Aerospace logo on the right.
    client_logo_path = db.Column(db.String(255), default='')
    ...
    def to_dict(self):
        return {
            ...
            'client_logo_url': f'/static/{self.client_logo_path}' if self.client_logo_path else '',
